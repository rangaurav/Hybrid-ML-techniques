...
evaluator = function(subset) {
  ...
  results = sapply(1:k, function(i) {
    ...
    tree  = ksvm(as.simple.formula(subset, "cluster"), train, kernel="rbfdot", prob.model=TRUE)
...
roc.multi <- multiclass.roc(testDataset$cluster, Predicted)
...
eval = function(attr.subset) {
  ...
  results = sapply(1:k, function(i) {
    ...
    tree = ksvm(formula, train, kernel="rbfdot", prob.model=TRUE)
    error.rate = sum(test$cluster != round(predict(tree, test),0)) / nrow(test)
